...
function CompanyJobs({ jobs }: Props) {
  ...
  const onClick = (companyName: string) => {
    logEvent(confUser?.email, `clicked job post ${companyName}_job`);
  };
  ...
  return (
    <div className={styles.grid}>
      {jobs.map(job => (
        <a
          onClick={() => onClick(job.companyName)}
          onKeyPress={() => onClick(job.companyName)}
          key={job.id}
